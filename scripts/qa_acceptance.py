...
health = sh("cd ~/news-project && ./venv/bin/python -c \"import sqlite3; con=sqlite3.connect('file:news-data/database/news.db?mode=ro', uri=True); print(con.execute(\\\"SELECT COUNT(*) FROM articles WHERE content IS NOT NULL AND fetched_at >= strftime('%Y-%m-%dT%H:%M:%SZ','now','-3 hours')\\\").fetchone()[0])\"")
check("S2 近3h 正文写入 >=1 (fetched_at 口径)", health.strip().isdigit() and int(health.strip()) >= 1, health.strip())
